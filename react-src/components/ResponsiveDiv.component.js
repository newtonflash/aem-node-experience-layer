...
const getDefaultMatch = mqName => {
  const currentScreen = window.RESPONSIVE_DIV_SCREEN_FALLBACK || 'lg';
  const currentGrid = gridDimensions[currentScreen];
  const mqScreen = mqName.slice(mqName.length - 2).toLowerCase();
  const mqGrid = gridDimensions[mqScreen];
  const mqMode =
    mqName === mqScreen
      ? 'eq'
      : mqName.indexOf('min') === 0
      ? 'min'
      : mqName.indexOf('max') === 0
      ? 'max'
      : undefined;
  if (mqMode === 'eq') {
    return currentScreen === mqScreen;
  }
  if (mqMode === 'min') {
    return currentGrid.minScreenWidth >= mqGrid.minScreenWidth;
  }
  if (mqMode === 'max') {
    return currentGrid.minScreenWidth <= mqGrid.minScreenWidth;
  }
  throw new TypeError(`Unknown mqName: ${mqName}`);
};
